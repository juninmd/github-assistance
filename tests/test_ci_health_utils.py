...
def test_remediate_pipeline_creates_vibe_code_opencode_task():
    ...
    agent.create_opencode_task.return_value = {"status": "task_created", "task_id": "t1", "task_url": "http://localhost:3000/tasks/t1"}
...
def test_remediate_pipeline_returns_none_when_opencode_raises():
    ...
    repo = MagicMock()
    repo.full_name = "owner/repo"
    agent.create_opencode_task.side_effect = RuntimeError("unexpected")
    failures = [{"name": "CI", "conclusion": "failure", "url": "https://github.com/run/1"}]
