...
class TestJulesTrackerAgentCoverage(unittest.TestCase):
    ...
    def test_run_session_not_in_allowlist(self):
        ...
        with patch(
            "src.agents.jules_tracker.utils.extract_repository_name", return_value="other/repo"
        ):
            result = self.agent.run()
            self.assertEqual(len(result["answered_questions"]), 0)
    ...
    def test_run_process_session_exception(self):
        ...
        self.jules_client.list_sessions.return_value = [{"id": "1", "state": "IN_PROGRESS"}]
        with patch(
            "src.agents.jules_tracker.utils.extract_repository_name", return_value="owner/repo"
        ):
    ...
    def test_run_session_with_url(self):
        ...
        self.jules_client.list_sessions.return_value = [
            {"id": "1", "state": "IN_PROGRESS", "url": "http://session"}
        ]
        with patch(
            "src.agents.jules_tracker.utils.extract_repository_name", return_value="owner/repo"
        ):
            with patch(
                "src.agents.jules_tracker.utils.get_pending_question", return_value="question"
            ):
