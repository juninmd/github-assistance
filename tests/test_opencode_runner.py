...
class TestOpencodeRunner(unittest.TestCase):
    ...
    @patch("src.agents.opencode_runner.agent_utils.get_free_opencode_model")
    @patch("src.agents.opencode_runner.tempfile.TemporaryDirectory")
    @patch("src.agents.opencode_runner.subprocess.run")
    def test_run_on_repo_returns_timeout_status_when_opencode_times_out(self, mock_run, mock_tmpdir, mock_get_model):
        mock_tmpdir.return_value.__enter__.return_value = "/tmp/repo"
        mock_get_model.return_value = "opencode/test-free"
    ...
    @patch("src.agents.opencode_runner.tempfile.TemporaryDirectory")
    @patch("src.agents.opencode_runner.subprocess.run")
    def test_run_on_repo_retries_with_fallback_model_and_opens_pr(self, mock_run, mock_tmpdir, mock_get_model):
        mock_tmpdir.return_value.__enter__.return_value = "/tmp/repo"
        mock_get_model.return_value = "opencode/test-free"
