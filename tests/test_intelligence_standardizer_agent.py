...
def test_analyze_intelligence_missing_all(mock_agent):
    ...
    repo.get_contents.side_effect = Exception(
        "Not found"
    )  # Simplification of UnknownObjectException for test
...
@patch(
    "src.agents.intelligence_standardizer.agent.IntelligenceStandardizerAgent.load_jules_instructions"
)
@patch(
    "src.agents.intelligence_standardizer.agent.IntelligenceStandardizerAgent.create_jules_session"
)
def test_process_repository_triggers_session(mock_create, mock_load, mock_agent):
    ...
    mock_agent._analyze_intelligence = MagicMock(
        return_value={
