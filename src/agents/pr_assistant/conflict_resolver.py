...
def _resolve_with_opencode(content: str) -> tuple[str | None, str]:
    ...
    model = agent_utils.get_random_free_opencode_model()
    prompt = (
    ...
    try:
        ...
        return None, ""
    if result.returncode != 0 and model != agent_utils._OPENCODE_DEFAULT_FREE_MODEL:
        model = agent_utils._OPENCODE_DEFAULT_FREE_MODEL
        try:
            result = subprocess.run(
                ["opencode", "run", "--model", model, prompt],
                capture_output=True,
