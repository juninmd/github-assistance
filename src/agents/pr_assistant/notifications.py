...
def _silent(fn, *args, **kwargs):
    """Run fn and silently ignore exceptions (notifications are best-effort)."""
    try:
        return fn(*args, **kwargs)
    except Exception:
        logger.debug("Silent notification call failed", exc_info=True)
...
def notify_conflict_resolved(github_client, telegram, pr, msg: str) -> None:
    ...
    _silent(github_client.comment_on_pr, pr, comment)
...
def notify_conflicts(github_client, telegram, pr, issue_comments: list | None = None) -> None:
    ...
    try:
        ...
        logger.debug("notify_conflicts github check failed", exc_info=True)
    ...
    _silent(telegram.send_message, text, parse_mode="HTML")
...
def notify_merge_failed(
    ...
    try:
        ...
        logger.debug("notify_merge_failed github check failed", exc_info=True)
...
def notify_pipeline_pending(
    ...
    try:
        ...
        logger.debug("notify_pipeline_pending github check failed", exc_info=True)
...
def warn_pipeline_failure(
    results: dict[str, Any], pr, status: dict, github_client, issue_comments: list | None = None
) -> None:
