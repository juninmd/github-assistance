...
def _build_header_lines(agent_name: str, results: dict[str, Any], esc) -> list[str]:
...
def _build_all_agent_lines(results: dict[str, Any], esc) -> list[str]:
    lines: list[str] = []
    success_count = 0
    fail_count = 0
...
def _build_senior_dev_lines(results: dict[str, Any], esc) -> list[str]:
    lines: list[str] = []
...
def _build_processed_failed_lines(results: dict[str, Any], esc) -> list[str]:
    lines: list[str] = []
...
def _build_single_agent_lines(agent_name: str, results: dict[str, Any], esc) -> list[str]:
    lines: list[str] = []
    if "error" in results:
        ...
        if agent_name == "senior-developer":
            lines.extend(_build_senior_dev_lines(results, esc))
        lines.extend(_build_processed_failed_lines(results, esc))
    return lines
...
def send_execution_report(
    ...
    lines = _build_header_lines(agent_name, results, esc)

    if agent_name == "all":
        lines.extend(_build_all_agent_lines(results, esc))
    else:
        lines.extend(_build_single_agent_lines(agent_name, results, esc))
